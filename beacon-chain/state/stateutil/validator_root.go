...
func merkleizeFlatArrayArmv8Neon(vec [][32]byte,
	depth uint8,
	...
	if len(vec)%2 == 1 {
		hash_tree[last-1] = hash.Hash2ChunksNeon(vec[len(vec)-1], zero_hash_array[0])
	}
	for {
		dist := last - first
		if dist < 2 {
		...
		if dist%2 != 0 {
			hash_tree[last-1] = hash.Hash2ChunksNeon(hash_tree[first-1], zero_hash_array[height])
		}
		height++
	}
	for {
		...
		hash_tree[last] = hash.Hash2ChunksNeon(hash_tree[last-1], zero_hash_array[height])
		last++
		height++
	}
...
func Uint64ListRootWithRegistryLimitArmv8Neonx1(balances []uint64, zero_hash_array [][32]byte) ([32]byte, error) {
	...
	balancesRootsRoot := merkleizeFlatArrayArmv8Neon(balancesChunks, 38, hash.PotuzHasherArmv8Neonx1Chunks, zero_hash_array)

	return hash.MixinLengthArmv8Neon(balancesRootsRoot, uint64(len(balances))), nil
}

func Uint64ListRootWithRegistryLimitArmv8Neonx4(balances []uint64, zero_hash_array [][32]byte) ([32]byte, error) {
	// assume len(balances) is multiple of 4 for this benchmark
	lenChunks := len(balances) / 4
	balancesChunks := make([][32]byte, lenChunks)
	for i := 0; i < lenChunks; i++ {
	...
	balancesRootsRoot := merkleizeFlatArrayArmv8Neon(balancesChunks, 38, hash.PotuzHasherArmv8Neonx4Chunks, zero_hash_array)
